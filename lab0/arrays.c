...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer: when the size of data exceed the array size, segment
  fillArray(array, 10);
  ...
  // Answer: because the memory address is aligned by the size of data
  fillArray(&value, 1);
  ...
  // C++ programming.
  // Answer: Memory will be definitely lost without free()
  free(heap_array);
  ...
  // variable instead of the . operator).
	FourInts* heap_array_four = (FourInts*) malloc(sizeof(FourInts));
	fillArray((int*)heap_array_four, 4);	
	assert(heap_array_four->a == 2);
  assert(heap_array_four->b == 5);
  assert(heap_array_four->c == 8);
  assert(heap_array_four->d == 11);

	free(heap_array_four);
  return 0;
}
